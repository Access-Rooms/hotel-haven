...
export function Header() {
  ...
  const { selectedHotel, selectedLocation, isMultiHotel } = useBooking();
  const displayName = selectedHotel?.name || companyConfig.name;
  const displayLocation = selectedLocation?.name || 'Select Location';
  const phone = selectedHotel?.phone || companyConfig.phone;
        ...
        <nav className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Button variant={isScrolled ? 'default' : 'heroOutline'} size="default" asChild>
              <Link to={selectedHotel ? `/hotels/${selectedHotel.slug}/rooms` : '/locations'}>Book Now</Link>
            </Button>
          </div>
